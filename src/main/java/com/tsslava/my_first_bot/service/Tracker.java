...
public class Tracker {
     ...
     * Apikey
     */
    private String Apikey = "2998620f-96c5-4da9-b160-7d313152c83c";
    ...
    public String trackInfo(String string) throws Exception {
        String details;
        String currentStatus;
        String date;
        String status;
        int error;
        String urlStr = "/china-post/" + string;
        ...
        error = jsonObjectMeta.getInt("code");
        if (error == 200) {
            JSONObject jsonObjectData = jsonObject.getJSONObject("data");
            JSONObject jsonObjectDestinationInfo = jsonObjectData.getJSONObject("destination_info");
            JSONArray jsonArrayTrackInfo = jsonObjectDestinationInfo.getJSONArray("trackinfo");
            JSONObject checkpoint_status = jsonArrayTrackInfo.getJSONObject(0);
            date = "Дата: " + checkpoint_status.getString("Date");
            details = "Текущее нахождение: " + checkpoint_status.getString("Details") + "\n";
            currentStatus = "Текущий статус: " + checkpoint_status.getString("StatusDescription") + "\n";
            status = details + currentStatus + date;
            return status;
        }else if (error == 4031) {
            return "Неверный номер почтового отслеживания!";
        }
